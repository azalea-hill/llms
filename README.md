...
- The three main aliases all resolve to `LMS_MAIN` so `/model` cannot land on an unloaded model; `LMS_MAIN` is the one thing to change to compare models. Haiku is `LMS_SMALL`. Claude Code uses haiku for background work (titles, summaries); `CLAUDE_CODE_SUBAGENT_MODEL=haiku` would send subagents there too, but the launcher's tool set omits `Agent`, so no subagents run unless `--tools` is overridden. In practice the haiku model gets one two-message request per session, the title call, answered in about a second.
...
- `CLAUDE_CODE_MAX_CONTEXT_TOKENS=200000` tells Claude Code the window for an unrecognized model ID so it compacts in time; `CLAUDE_CODE_DISABLE_1M_CONTEXT=1` makes that apply even when the account's default model alias carries a `[1m]` suffix. `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=70` compacts at 70% of the window; compaction is itself a model request over the whole transcript (62 s on the MoE in one session), so on a local model it should happen with headroom. The declared window is deliberately below the models' effective 262K on LM Studio and well above what Claude Code's estimator implies: with no `count_tokens` endpoint, Claude Code estimates from characters, and on tool-result-heavy content the estimate ran 1.94x the real count (a session compacted at 80K estimated when LM Studio had seen 41K). At 200K and 70%, compaction fires at 140K estimated, roughly 70-105K real depending on content.

Flags the launcher adds unless you pass your own: `--effort medium` (a flag rather than `CLAUDE_CODE_EFFORT_LEVEL`, because the env var is a hard override that `/effort` cannot change mid-session), `--permission-mode acceptEdits` (auto mode cannot work locally: its classifier requests follow `ANTHROPIC_BASE_URL` and need a Claude model), `--tools Bash,Read,Edit,Write,Glob,Grep,AskUserQuestion,TodoWrite,TaskCreate,TaskGet,TaskList,TaskUpdate,EnterPlanMode,ExitPlanMode` (file and shell tools, the model's task list, and plan mode so the model can propose before editing; no `Agent`, so no subagents, and no `WebSearch`, which runs on Anthropic's servers), `--strict-mcp-config`, and a `--session-id` (skipped when resuming). A trial that appended a short system prompt (read files in ranges, do not re-read after editing, prefer `git diff --stat`) cut a session's tool-result volume by ~40%; it was removed to keep the launcher plain, and `--append-system-prompt` is the flag to use if that guidance is wanted again. The tool restriction matters for context: on a non-first-party base URL Claude Code sends every tool definition in full on every request, and the default built-in set is 21 tools and about 46 KB of JSON before `Artifact` and any connector tools; this set is about 12 KB. A local model executing a plan needs none of the rest. Unknown tool names in `--tools` are ignored, so the list is safe across versions.
...
`/context` in a local session is an estimate, not a count: LM Studio has no `count_tokens`, so Claude Code falls back to a character-based guess that runs 1.3x above the real count on a fresh session and up to about 2x once tool results (file contents, diffs) dominate. Compaction triggers off the estimate, so it fires early rather than late; the declared window is set with that in mind. Session `36c8420c` compacted at 80,378 estimated against 41,455 real after the model read a 48K-character file whole, edited it 16 times, read it whole again, and ran a full `git diff` on it; the model's habit of re-reading whole files is the biggest variable cost.
